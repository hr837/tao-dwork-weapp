...
Component({
  ...
  properties: {
    ...
    begin: {
      type: String,
      value: '2020-01-01' //年月日字串
    },

    deadline: { // 最后有效期, 如果用户没设置选择截止日期,那默认是今天
      type: String,
      value: '' //年月日字串
    },
  ...
  observers: {
    ...
    'markBlue': function (mark) {
      ...
      this.setData({ markBlueObj: obj })
    },

    'markOrange': function (mark) {
      let obj = {} as any
      if (mark instanceof Array) {
        for (let item of mark) {
          //用来在渲染中判断哪些日期需要加红点
      ...
      this.setData({ markOrangeObj: obj })
    },
  ...
  ready() {
    console.log("---  markBlue data:", this.properties.markBlue);
    console.log("---  markOrange data:", this.properties.markOrange);
  ...
  methods: {
    ...
    onSelectDate(e: any) {
      ...
      return true // 为了消除警告: 并非所有代码路径都返回值。
    },
    ...
    onBackToToday() {
      ...
      //回传当天
      this.triggerEvent('getdate', { selected: this.data.today.ymd  })
    },
    ...
    onCalChange(e: any) {
      ...
      let { ymd } = this.data.calendar;
      console.log('-- onCalChange:', year, month, ymd);
      //回传当前月份第一天日期
      this.triggerEvent('changeMonth', { monthFirtsDate: ymd })
    },
    ...
    _setCalendarData(targetDate = new Date()) {
      ...
      let { year, month, date, day, y, m, d, ymd } = this._getDateKeys(targetDate),
        ...
        firstWeekDay = {} as any,  //日历中显示的第一周的第一天日期对象
        allDays = [], //全部相关的一系列日期对象
        calendar = { y, m, d, ymd, day, month, date, year, weeks: [] } as any //渲染数据
      ...
      console.log('-- 日历渲染数据', calendar); //todo 这里可以查看渲染日历的数据
    },
     ...
     * @private
     */
    _calShowToggle(isShow: any = undefined) {
    ...
    * 左补位
    * */
    _strLeftPad(str: any, len: any, pad = '0') {
    ...
    _getDateKeys(targetDate = new Date()) {
      let year = targetDate.getFullYear(),
        ...
        y = String(year),
        m = this._strLeftPad(String(month + 1), 2, '0'),
        d = this._strLeftPad(String(date), 2, '0'),
        ymd = `${y}-${m}-${d}`
